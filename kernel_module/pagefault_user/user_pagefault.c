...
{
	fprintf(stderr, "thread id=%d, print_virtual_memory() - reading from %p\n", gettid(), memory);
	fprintf(stderr, "thread id=%d, content is: %s", gettid(), (char *)memory);
	return NULL;
}
...
int main (int argc, char **argv)
{
    ...
    int err = 0;
    err = ioctl(fd, UFFDIO_REGISTER,  &reg);
    assert(!err && "++ ioctl(fd, UFFDIO_REGISTER, ...) failed: %m\n");
	//assert(reg.ioctls == UFFD_API_RANGE_IOCTLS && "++ unexpected UFFD ioctls.\n");
	...
	char data[PAGESIZE] = "some default string\n";
	struct pollfd evt = { .fd = fd, .events = POLLIN };
	while (poll(&evt, 1, 10) > 0) {
		/* unexpected poll events */
		assert(!(evt.revents & POLLERR) && "++ POLLERR\n");
		assert(!(evt.revents & POLLHUP) && "++ POLLHUP\n");
		...
		fprintf(stderr, "thread id=%d, handling user pagefault. Copying some content to %p\n", gettid(), place);
