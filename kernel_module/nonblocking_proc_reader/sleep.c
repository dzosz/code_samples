...
MODULE_AUTHOR("STOLEN FROM THE INTERNET");
...
static int module_open(struct inode *inode, struct file *file)
{
	...
	printk(KERN_INFO "module try to open\n");
	while (Already_Open) {
		int i, is_sig = 0;
...
//‘int (*)(struct inode *, int)’ from incompatible pointer type 
//‘int (*)(struct inode *, int,  struct nameidata *)
static int module_permission(struct inode *inode, int op)//, struct nameidata *nd)
{
    ...
    kuid_t rootUid;
    rootUid.val = 0;
	if (op == 4 || (op == 2 && uid_eq(current_euid(), rootUid)))
		return 0;
 ...
 * we just put NULL).
 */
static const struct inode_operations Inode_Ops_4_Our_Proc_File = {
...
int init_module()
{
	...
	Our_Proc_File = proc_create(PROC_ENTRY_FILENAME, 0644, NULL, &pops);
